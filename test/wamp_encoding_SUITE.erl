...
hello_json_test(_) ->
    ...
    {[hello], <<>>} = wamp_encoding:decode_message_names({ws, text, json}, Bin),
...
welcome_json_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[welcome], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
abort_json_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[abort], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
challenge_json_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[challenge], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
authenticate_json_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[authenticate], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
goodbye_json_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[goodbye], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
error_json_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[error], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
publish_json_test(_) ->
    ...
    {[publish], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
published_json_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[published], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
subscribe_json_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[subscribe], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
subscribed_json_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[subscribed], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
unsubscribe_json_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[unsubscribe], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
unsubscribed_json_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[unsubscribed], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
event_json_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[event], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
call_json_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[call], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
cancel_json_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[cancel], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
result_json_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[result], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
register_json_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[register], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
registered_json_2_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[registered], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
unregister_json_3_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[unregister], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
unregistered_json_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[unregistered], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
invocation_json_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[invocation], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
interrupt_json_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[interrupt], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
yield_json_test(_) ->
    ...
    Bin = wamp_encoding:encode(M, json),
    {[yield], <<>>} = wamp_encoding:decode_message_names(
        {ws, text, json}, Bin),
    {[M], <<>>} = wamp_encoding:decode({ws, text, json}, Bin).
...
hello_msgpack_test(_) ->
    ...
    {[hello], <<>>} = wamp_encoding:decode_message_names(
